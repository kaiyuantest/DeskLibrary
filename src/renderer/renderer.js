...
const els = {
  ...
  shortcutCaptureModalOverlay: document.getElementById('shortcutCaptureModalOverlay'),
  closeShortcutCaptureModalBtn: document.getElementById('closeShortcutCaptureModalBtn'),
  ...
  shortcutCaptureValue: document.getElementById('shortcutCaptureValue'),
  shortcutCaptureCancelBtn: document.getElementById('shortcutCaptureCancelBtn'),
...
let shortcutCaptureContext = null;
...
• 图片等非文本内容不受此规则影响`
  ...
  copyThenKey: {
    title: '复制后再按快捷键收藏',
    subline: '支持单键或组合键（如 K / Ctrl+K / Shift+Alt+S）',
    body: '复制后进入观察窗口时，可按所填单键或组合键立即保存当前剪贴板。若在观察窗口内按下纯 Alt（不带其它修饰键），会将当前内容加入「常用」分类。此项由全局按键监听实现，不是 Electron 全局热键。'
  },
  deleteLastCapture: {
    title: '删除上次收藏',
...
function normalizeModifierKeyName(rawKey) {
  const key = String(rawKey || '').toLowerCase();
  if (key === 'control' || key === 'ctrl') return 'Ctrl';
  if (key === 'alt') return 'Alt';
  if (key === 'shift') return 'Shift';
  if (key === 'meta' || key === 'os' || key === 'win' || key === 'command') return 'Super';
  return '';
}

function shortcutKeyTokenFromEvent(event) {
...
function formatShortcutFromEvent(event, mode = 'accelerator') {
  const modifiers = [];
  ...
  const keyToken = shortcutKeyTokenFromEvent(event);
  const modifierToken = normalizeModifierKeyName(keyToken);
  if (modifierToken) {
    if (mode === 'postcopy') {
      if (modifiers.length === 1) return modifiers[0];
      if (modifiers.length > 1) return modifiers.join('+');
    }
    return '';
  }

  if (!keyToken) {
    if (mode === 'postcopy' && modifiers.length > 0) {
...
function stopShortcutCapture() {
  if (!shortcutCaptureContext) return;
  const { button } = shortcutCaptureContext;
  if (button) {
    button.classList.remove('capturing');
    if (button.dataset.shortcutLabel) {
      button.textContent = button.dataset.shortcutLabel;
    }
  }
  if (els.shortcutCaptureModal) {
    els.shortcutCaptureModal.classList.add('hidden');
  }
  if (els.shortcutCaptureConfirmBtn) {
    els.shortcutCaptureConfirmBtn.disabled = true;
  }
  if (els.shortcutCaptureValue) {
    els.shortcutCaptureValue.textContent = '未检测到按键';
  }
  shortcutCaptureContext = null;
}

function startShortcutCapture(button, input, mode = 'accelerator') {
  stopShortcutCapture();
  if (!button || !input) return;
  shortcutCaptureContext = { button, input, mode, capturedValue: '' };
  if (!button.dataset.shortcutLabel) {
    button.dataset.shortcutLabel = button.textContent || '⌨';
  }
  button.textContent = '●';
  button.classList.add('capturing');
  if (els.shortcutCaptureTitle) {
    els.shortcutCaptureTitle.textContent = '录制快捷键';
  }
  if (els.shortcutCaptureHint) {
    els.shortcutCaptureHint.textContent = mode === 'postcopy'
      ? '请按单键或组合键（例如 K / Ctrl+K），按 Esc 取消'
      : '请按快捷键组合（例如 Ctrl+Alt+S），按 Esc 取消';
  }
  if (els.shortcutCaptureValue) {
    els.shortcutCaptureValue.textContent = '未检测到按键';
  }
  ...
  els.shortcutCaptureModal?.classList.remove('hidden');
}

function handleShortcutCaptureKeydown(event) {
  if (!shortcutCaptureContext || els.shortcutCaptureModal?.classList.contains('hidden')) return;
  event.preventDefault();
  event.stopPropagation();
  ...
  if (event.key === 'Enter' && shortcutCaptureContext.capturedValue) {
    confirmShortcutCapture();
    return;
  }
  const nextValue = formatShortcutFromEvent(event, shortcutCaptureContext.mode);
  if (!nextValue) return;
  shortcutCaptureContext.capturedValue = nextValue;
  if (els.shortcutCaptureValue) {
    els.shortcutCaptureValue.textContent = nextValue;
  }
  if (els.shortcutCaptureConfirmBtn) {
    els.shortcutCaptureConfirmBtn.disabled = false;
  }
}

function confirmShortcutCapture() {
  if (!shortcutCaptureContext) return;
  const value = String(shortcutCaptureContext.capturedValue || '').trim();
  if (!value) return;
  shortcutCaptureContext.input.value = value;
  shortcutCaptureContext.input.dispatchEvent(new Event('input', { bubbles: true }));
  stopShortcutCapture();
}
...
els.resetSettingsBtn?.addEventListener('click', async () => {
  if (!window.confirm('确认还原设置为默认值吗？当前已保存的设置将被覆盖。')) {
    return;
  }
  const result = await window.deskLibrary.resetSettings();
  if (!result || result.ok === false) {
    alert((result && result.message) || '还原默认设置失败');
    return;
  }
  state.settings = result.settings || (await window.deskLibrary.getInitialData()).settings || {};
  state.browserScanRoot = state.settings.selfBuiltWorkspaceDir
    || state.settings.selfBuiltUserDataRoot
    || state.settings.browserScanRoot
    || state.browserScanRoot
...
els.settingsPage?.addEventListener('click', (event) => {
  const captureTrigger = event.target.closest('[data-shortcut-target]');
  if (captureTrigger) {
    event.preventDefault();
    event.stopPropagation();
    const targetId = captureTrigger.getAttribute('data-shortcut-target');
    const mode = captureTrigger.getAttribute('data-shortcut-mode') || 'accelerator';
    const input = document.getElementById(targetId);
    if (input) {
      startShortcutCapture(captureTrigger, input, mode);
    }
    return;
  }
...
window.addEventListener('blur', () => stopShortcutCapture());
els.shortcutCaptureConfirmBtn?.addEventListener('click', () => confirmShortcutCapture());
els.shortcutCaptureCancelBtn?.addEventListener('click', () => stopShortcutCapture());
els.closeShortcutCaptureModalBtn?.addEventListener('click', () => stopShortcutCapture());
els.shortcutCaptureModalOverlay?.addEventListener('click', () => stopShortcutCapture());
